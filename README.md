...
> **R:** Porque programa.i incluye todo el codigo que en vsc se reduce. por ejemplo las bibliotecas #include y el reemplazo de macros, lo cual hace que tenga mucha mas lineas de codigo que el programa original
...
> **R:** En teoria como el texto  "Archivo fuente principal" esta dentro del programa.c. El procesador sigue manteniendo ese mensaje lo cual el grep pueda encontrarlo.
...
> **R:** en primer comando no compila y el segundo devuelve: printf("[DEBUG] %s\n", ("Iniciando main"));
...
> **R:** Las lineas indican al compilador que el codigo proviene de ese archivo y corresponde a esa linea original y asi saber de donde viene el origen del codigo por ejemplo printf proviene del archivo stdio.h
...
> **R:**
Prepar la pila y guardar el contexto de la funcion anterior.
---
...
en programa.o no es definida, en cambio. En matematica.o la funcion si esta implementada
...
> **R:**
bash: ./programa.o: cannot execute binary file: Exec format error
...
Por que vienen de bibliotecas de tipo dinamicas. Solo se definen cuando estan en tiempo de ejecucion
...
> **R:** Cuadrado(X) es un reemplazo de texto donde crea que el preprocesador sea necesario. por ejemplo al decir cuadrado de x es lo mismo que x*x y desparece en la etapa de prepocesamiento ya que en el codigo queda expandida (x*x), en cambio sumar(a, b) es un codigo ya compilado (en tiempo de ejecucion). La funcion no desparece en la primera etapa  si no compila, luego manda las instrucciones en codigo maquina y luego la realiza. desaparecia o en el ensamblado o enlazado.
...
> **R:** 
T quiere decir que esta en la seccion .text y es un codigo ejecutable
D quiere decir que esta en la seccion .data y es una variable global o estatica ya definida.
...
> **R:**
        ntdll.dll => /c/Windows/SYSTEM32/ntdll.dll (0x7ffb27f20000)
        ntdll.dll => /c/Windows/SysWOW64/ntdll.dll (0x77c00000)
        wow64.dll => /c/Windows/System32/wow64.dll (0x7ffb26170000)
        wow64base.dll => /c/Windows/System32/wow64base.dll (0x7ffb265a0000)
        wow64win.dll => /c/Windows/System32/wow64win.dll (0x7ffb27c70000)
        wow64con.dll => /c/Windows/System32/wow64con.dll (0x7ffb265b0000)

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:** Porque es como que gcc piensa que libc ya viene por defecto, por lo que no necesitaria expecificar.
